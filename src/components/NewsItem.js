...
export default function NewsItem(props) {
  return (
    <div className="item-style container p-0" style={{ height: "100%" }}  >
      <div className="card item-style" style={{ height: "100%" }}>
        ...
        <div className="card-body d-flex flex-column justify-content-between">
          ...
          <div><p className="card-text"><small className="text-muted"><Moment fromNow>{new Date(props.date).toLocaleString()}</Moment> </small></p>
            <button disabled={props.link ? false : true} className="btn btn-light" ><a rel="noreferrer" href={props.link} target="_blank" style={{ color: "white", textDecoration: "none" }}>Read More</a></button>
          </div>
        </div>
      </div>
    </div>
